...
namespace Step18
{
  ...
  const types::material_id first_domain_id = 1,
                           second_domain_id = 2;
  bool stress_debug_flag = false;
  bool dz_flag = false;
  ...
  private:
    ...
    static const SymmetricTensor<4,dim> stress_strain_tensor_metal;
    static const SymmetricTensor<4,dim> stress_strain_tensor_plex;
                                ...
                                const types::material_id material_id) const
  {
    ...
    const double g   = 9.81;
    double rho = 7700;
    if (material_id == second_domain_id) {
      //rho = 3700;
      rho = 1500;
    }
  ...
  TopLevel<dim>::stress_strain_tensor_metal
    = get_stress_strain_tensor<dim> (/*lambda = */ 9.695e10,
                                                   /*mu     = */ 7.617e10);

  template <int dim>
  const SymmetricTensor<4,dim>
  TopLevel<dim>::stress_strain_tensor_plex
    = get_stress_strain_tensor<dim> (/*lambda = */ 5.45e9,
                                                   /*mu     = */ 1.48e9);

//    = get_stress_strain_tensor<dim> (/*lambda = */ 5.45e10,
//                                                 /*mu     = */ 3.48e10);
  ...
  template <int dim>
  void TopLevel<dim>::make_grid_composite()
  {
    std::vector<types::material_id> material_map(10);
       ...
       cell != dof_handler.end(); ++cell)
    {
      ...
      for (unsigned int i = 0; i < material_map.size(); ++i) 
      {
        if ((std::fabs(cell->center()[dim-1]) >= i/*/2.0*/) && (std::fabs(cell->center()[dim-1]) < (i + 1)/*/2.0*/)) 
        {
      ...
      if (cell->subdomain_id() == this_mpi_process)
        {
            ...
            for (unsigned int j=0; j<dofs_per_cell; ++j)
              for (unsigned int q_point=0; q_point<n_q_points;
                   ++q_point)
                {
                  ...
                  if (cell->material_id() == first_domain_id)
                    cell_matrix(i,j)
                    += (eps_phi_i * stress_strain_tensor_metal * eps_phi_j
                        *
                        fe_values.JxW (q_point));
                  else
                    cell_matrix(i,j)
                    += (eps_phi_i * stress_strain_tensor_plex * eps_phi_j
                        *
                        fe_values.JxW (q_point));
                }
          ...
          body_force.vector_value_list (fe_values.get_quadrature_points(),
                                        body_force_values, cell->material_id());
  ...
  void TopLevel<dim>::output_results () const
  {
    ...
    Tensor<2,dim> cause_i_dont_know_better_way({{0,0,0}, {0,0,0}, {0,0,0}});
    SymmetricTensor<2,dim> zero_stress(cause_i_dont_know_better_way);

    Vector<double> materials(triangulation.n_active_cells());
    Vector<double> norm_of_stress (triangulation.n_active_cells());
    Vector<double> sigmaxx(triangulation.n_active_cells());
    Vector<double> sigmayy(triangulation.n_active_cells());
    Vector<double> sigmazz(triangulation.n_active_cells());
    Vector<double> sigmaxy(triangulation.n_active_cells());
    Vector<double> sigmaxz(triangulation.n_active_cells());
    Vector<double> sigmayz(triangulation.n_active_cells());
    {
      ...
      for (unsigned int index=0; cell!=endc; ++cell, ++index) {
        if(!dz_flag) {
          for (unsigned int f=0; f<GeometryInfo<dim>::faces_per_cell; ++f)
            if (cell->face(f)->at_boundary() && 1 == cell->face(f)->boundary_id())
            {
              std::cout << cell->face(f)->center()[0] << "  " 
                        << cell->face(f)->center()[1] << "  " 
                        << cell->face(f)->center()[2] << std::endl; 
              dz_flag=true;
            }
        }
        ...
        if (cell->subdomain_id() == this_mpi_process)
          {
            ...
            //stress(index) = accumulated_stress / quadrature_formula.size(); 
            norm_of_stress(index)
              = (accumulated_stress /
                 quadrature_formula.size()).norm();
            sigmaxx(index) = accumulated_stress[0][0];
            sigmayy(index) = accumulated_stress[1][1];
            sigmazz(index) = accumulated_stress[2][2];
            sigmaxy(index) = accumulated_stress[0][1];
            sigmaxz(index) = accumulated_stress[0][2];
            sigmayz(index) = accumulated_stress[1][2];
            if(!stress_debug_flag) {
              stress_debug_flag = true;
              for(unsigned int i = 0; i < 3; ++i) {
                for(unsigned int j = 0; j < 3; ++j) {
                  std::cout << accumulated_stress[j][i] <<  " ";
                }
                std::cout << std::endl;
              }
            }
    ...
    //data_out.add_data_vector (stress, "stress", DataOut<dim>::type_dof_data, data_component_interpretation);
    data_out.add_data_vector (sigmaxx, "sigmaxx");
    data_out.add_data_vector (sigmayy, "sigmayy");
    data_out.add_data_vector (sigmazz, "sigmazz");
    data_out.add_data_vector (sigmaxy, "sigmaxy");
    data_out.add_data_vector (sigmaxz, "sigmaxz");
    data_out.add_data_vector (sigmayz, "sigmayz");
...
/*
    std::vector<types::material_id> material_type (triangulation.n_active_cells());
    unsigned int count = 0;
    for (typename Triangulation<dim>::active_cell_iterator
           cell = dof_handler.begin_active();
           cell != dof_handler.end(); ++cell)
    {
        material_type(count) = cell->material_id();
        ++count;
    }
    const Vector<int> materials(material_type.begin(),
                                material_type.end());
*/
    ...
    // number):
    std::string filename = res_dir_prefix + "solution-" + Utilities::int_to_string(timestep_no,4)
    ...
    if (this_mpi_process==0)
      {
        ...
        for (unsigned int i=0; i<n_mpi_processes; ++i)
          filenames.push_back (res_dir_prefix + "solution-" + Utilities::int_to_string(timestep_no,4)
        ...
        const std::string
        visit_master_filename = (res_dir_prefix + "solution-" +
        ...
        const std::string
        pvtu_master_filename = (res_dir_prefix + "solution-" +
                                Utilities::int_to_string(timestep_no,4) +
                                ".pvtu");
        ...
        std::ofstream pvd_output (res_dir_prefix + "solution.pvd");
          ...
          for (unsigned int q=0; q<quadrature_formula.size(); ++q)
            {
              ...
              if (cell->material_id() == first_domain_id) {
                const SymmetricTensor<2,dim>
                new_stress = (local_quadrature_points_history[q].old_stress
                    +
                    (stress_strain_tensor_metal *
